            			...
            			<form action="/board/saveContent" id="contentForm" method="post" enctype="multipart/form-data">
							...
							<div class="col-4">
	      					...
	      					<div id = "selectLinkDiv" style="display:none;">
					        	...
					        	<div class="col-4">
					        		<div>대표 이미지는 반드시 업로드하셔야 합니다.</div>
					        	<button id="submiter" class="btn btn-primary" type="button" style="padding-right:22px; background-color:#0062cc; margin-top:10px;">등록</button>		
	      					...
	      					<div id = "selectDetailDiv" >
		      					...
		      					<hr style="margin-top:20px;">
					        	<div class="col-4">
					        		<div>대표 이미지를 설정하시려면 이미지 첨부하신 뒤 썸네일을 클릭해주세요.</div>
					        	<button  id="submiter" class="btn btn-primary" type="button" style="padding-right:22px; background-color:#0062cc; margin-top:10px;">등록</button>		
...
var pasteThumbnail = function(imgLink){
	...
	for(let idx = 0; idx < imgLength; idx++){
	   ...
	   // console.log("2: "+ thumnail_linkFinal);
		...
		thumbnailHTML += "<img src="+thumnail_linkFinal+" class='thumnail_linkFinal' style='height:100px; width:100px; cursor:pointer; '>";	    		
...
$(document).ready(function() {
	...
	CKEDITOR.instances.CKeditor.on('change', function(ev) {
		...
		//이미지 링크 배열에 저장됨
		/*
		imgSrcArray = Array.from( new DOMParser().parseFromString( CKEDITOR.instances.CKeditor.getData(), 'text/html' )
			    .querySelectorAll( 'img' ) )
			    .map( img => img.getAttribute( 'src' ) );*/
		
		var a = new DOMParser().parseFromString( CKEDITOR.instances.CKeditor.getData(), 'text/html' ).querySelectorAll( 'img' )
	...
	$("#thumbnailDiv").on("click",".thumDiv", function(){
		...
		//대표이미지 설정... 시각적인 표시
		$(".titleImageDiv").html("");
		let plusHtml = "<input type='text' id = 'titleImageText' readonly value='대표이미지' style='width:70px; background-color:#C1FF6B;'>";
...
var writeFormSubmit = function(){
	...
	if($("#currentSelect").val() == "selectLinkWrite"){
		...
		if($("#titleImagePath").val() == ""){
	        alert("대표 이미지를 지정해야 합니다. 썸네일 중 하나를 클릭해주세요");
	        return false;			
		}
		...
		//대표 이미지를 본문에서는 숨김 처리 
		link = $("#titleImageText").parent().siblings('.thumnail_linkFinal').attr("src");
		
		let link_position = link.indexOf("s_");
	   	
	    //이미지 링크만 빼내기
	    let original_link = link.substring(link_position+2, link.length);
	    
	    let original_link_ch = original_link.replace("&","&amp;");
	    
	    //본문에서 링크와 일치하는 것을 찾아서 공백 처리
		let current_data = CKEDITOR.instances.CKeditor.getData();
	    
	    CKEDITOR.instances.CKeditor.setData(current_data.replace(original_link_ch,""));
		
	    let chenged_data = CKEDITOR.instances.CKeditor.getData();
	    $("#brd_content").val(chenged_data);
	}
